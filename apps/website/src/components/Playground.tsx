...
async function startTempblotLanguageClient(
  ...
  const worker = new Worker(
    new URL("../workers/tempblotLanguageServer.worker.ts", import.meta.url),
    {
      type: "module",
    },
  );
...
async function configureTextMate(
  monaco: Monaco,
  instance: editor.IStandaloneCodeEditor,
  themeName: string,
) {
  textMateConfiguration ??= (async () => {
    ...
    const registry = new Registry({
      ...
      loadGrammar: async (scopeName) =>
        grammarByScope.get(scopeName) ?? createPlainTextGrammar(scopeName),
    });
...
async function updateTextMateTheme(
  ...
  monaco.languages.setTokensProvider(
    "tempblot",
    createTextMateTokensProvider(loadedGrammar),
  );
...
function createTextMateTokensProvider(
  grammar: IGrammar,
): languages.EncodedTokensProvider {
  return {
    getInitialState() {
    ...
    tokenizeEncoded(line, state) {
      const result = grammar.tokenizeLine2(
        line,
        state instanceof TextMateState ? state.stack : INITIAL,
      );
      return {
...
function createTextMateTheme(themeName: string): IRawTheme {
  const theme =
    themeName === tempblotLightTheme ? lightPlusTheme : darkPlusTheme;
...
function registerLanguageProviders(
  ...
  if (capabilities.completionProvider) {
    disposables.push(
      monaco.languages.registerCompletionItemProvider("tempblot", {
  ...
  if (capabilities.hoverProvider) {
    disposables.push(
      monaco.languages.registerHoverProvider("tempblot", {
  ...
  if (capabilities.signatureHelpProvider) {
    disposables.push(
      monaco.languages.registerSignatureHelpProvider("tempblot", {
        ...
        async provideSignatureHelp(
          ...
          const signatureHelp = (await connection.sendRequest(
  ...
  if (capabilities.definitionProvider) {
    disposables.push(
      monaco.languages.registerDefinitionProvider("tempblot", {
  ...
  if (capabilities.semanticTokensProvider) {
    const semanticTokensProvider =
      capabilities.semanticTokensProvider as SemanticTokensOptions;
    disposables.push(
      monaco.languages.registerDocumentSemanticTokensProvider("tempblot", {
        getLegend() {
...
function markupToMarkdownStrings(value: Hover["contents"]): IMarkdownString[] {
  if (Array.isArray(value)) {
    return value
      .map(markupToMarkdownString)
      .filter((item): item is IMarkdownString => Boolean(item));
  }
...
function markupToMarkdownString(
  value:
    | string
    | MarkupContent
    ...
    if ("targetUri" in location) {
      return {
        ...
        originSelectionRange: location.originSelectionRange
          ? toMonacoRange(monaco, location.originSelectionRange)
          : undefined,
      };
    }
